...
void findMeanPtDifferenceInResponseMatrix(){
  ...
  for(auto centralityBin : comparedCentralityBin){
    ...
    for(auto jetPtBin : comparedJetPtBin){
      ...
      trueMean = hJetPt[iCentrality]->GetMean();

      // Calculate the measured mean jet pT in a selected bin. This is the mean before unfolding.
      measuredMean = hMeasuredMean[iCentrality]->GetBinContent(hMeasuredMean[iCentrality]->FindBin(jetPtBin.first + epsilon));

      // Print the information to the console:
      cout << "Results for centrality bin " << centralityBin.first << "-" << centralityBin.second << "  and jet pT bin " << jetPtBin.first << "-" << jetPtBin.second << endl;
      cout << "Measured mean: " << measuredMean << "    True mean: " << trueMean << "   Upshift: " << trueMean - measuredMean << endl;
