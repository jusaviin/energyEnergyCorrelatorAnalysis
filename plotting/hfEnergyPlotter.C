...
void hfEnergyPlotter(){
  ...
  TString inputFileName = "data/pPb/monteCarlo/pythiaEpos_8TeV_GenGen_pToPlusEta_genJets_eschemeAxis_nominalEnergyWeight_jetEtaCMcut_megaSkimMix_9filesMissing_processed_2026-02-13.root";
  ...
  // Define maps that connect bin indices to specific histograms
  std::map<std::pair<int,int>, TH1D*> hHFPlus;
  std::map<std::pair<int,int>, TH1D*> hHFMinus;
  std::map<std::pair<int,int>, TH1D*> hHFSum;
  std::map<std::pair<int,int>, TH2D*> hHFPlusVsHFMinus;

  // Binning variable for the histograms
  std::pair<int,int> currentBin;

  // Graphs with jet pT dependent mean HF sum
  std::map<int, TGraph*> gAverageHFSum;
  ...
  for(auto jetPtBin : comparedJetPtBin){
    ...
    for(int iNJet = 0; iNJet < nNJetBins+1; iNJet++){
      ...
      currentBin = std::make_pair(iJetPt, iNJet);

      // Read the histograms from the file
      hHFPlus[currentBin] = histograms->GetHistogramHFPlus(iJetPt, nJetIndex);
      hHFMinus[currentBin] = histograms->GetHistogramHFMinus(iJetPt, nJetIndex);
      hHFSum[currentBin] = histograms->GetHistogramHFSum(iJetPt, nJetIndex);
      hHFPlusVsHFMinus[currentBin] = histograms->GetHistogramHFPlusVsHFMinus(iJetPt, nJetIndex);
  ...
  for(auto jetPtBin : comparedJetPtBin){
    ...
    for(int iNJet = 1; iNJet <= nNJetBins; iNJet++){
      ...
      graphYpoints[nJetIndex].push_back(hHFSum[std::make_pair(iJetPt, iNJet)]->GetMean());
    } // Number of jets loop
  } // Jet pT loop
  ...
  currentBin = std::make_pair(nJetPtBins,0);
  TLine* zeroJetsLine = new TLine(comparedJetPtBin.at(0).first, hHFSum[currentBin]->GetMean(), comparedJetPtBin.at(comparedJetPtBin.size()-2).second, hHFSum[currentBin]->GetMean());
  ...
  TLine* estimatedShiftLine = new TLine(comparedJetPtBin.at(0).first, hHFSum[currentBin]->GetMean()+hfShiftValue, comparedJetPtBin.at(comparedJetPtBin.size()-2).second, hHFSum[currentBin]->GetMean()+hfShiftValue);
  estimatedShiftLine->SetLineColor(kBlack);
  estimatedShiftLine->SetLineStyle(2);
  TLine* estimatedShiftLineUp = new TLine(comparedJetPtBin.at(0).first, hHFSum[currentBin]->GetMean()+hfShiftValue+3, comparedJetPtBin.at(comparedJetPtBin.size()-2).second, hHFSum[currentBin]->GetMean()+hfShiftValue+3);
  estimatedShiftLineUp->SetLineColor(kBlack);
  estimatedShiftLineUp->SetLineStyle(2);
  TLine* estimatedShiftLineDown = new TLine(comparedJetPtBin.at(0).first, hHFSum[currentBin]->GetMean()+hfShiftValue-3, comparedJetPtBin.at(comparedJetPtBin.size()-2).second, hHFSum[currentBin]->GetMean()+hfShiftValue-3);
  ...
  if(draw2DHFmap){
    for(auto jetPtBin : comparedJetPtBin){
      ...
      // Make pair of the current bin
      currentBin = std::make_pair(iJetPt, nNJetBins);


      drawer->DrawHistogram(hHFPlusVsHFMinus[currentBin],"HF Pb going", "HF p going", jetPtString.Data(), "colz");
      diagonal->Draw();
  ...
  if(drawHFsum){
    for(auto jetPtBin : comparedJetPtBin){
      for(int iNJet = 0; iNJet < nNJetBins+1; iNJet++){
        ...
        drawer->DrawHistogram(hHFSum[currentBin], "HF energy", "Counts", " ");
        legend->AddEntry(hHFSum[currentBin], jetPtString.Data(), "l");
        legend->AddEntry((TObject*)0, nJetString.Data(), "");
        legend->AddEntry((TObject*)0, Form("Mean value: %.3f", hHFSum[currentBin]->GetMean()), "");
        legend->Draw();
      } // Number of jets loop
    } // Jet pT loop
  } // Draw 2D HF energy map
