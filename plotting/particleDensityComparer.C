...
void particleDensityComparer(){
  ...
  TString inputFileName[knDataTypes] = {"data/eecAnalysis_akFlowJets_noBadAcc_refConeWeight_wtaAxis_preprocessed_2022-10-31.root", "data/MinBiasHydjet_RecoGen_eecAnalysis_akFlowJet_MnD_eschemeAxis_noTrigger_preprocessed_2022-10-19.root"};
      ...
      for(int iTrackPt = firstStudiedTrackPtBinEEC; iTrackPt <= lastStudiedTrackPtBinEEC; iTrackPt++){
        for(int iJetPt = firstStudiedJetPtBinEEC[kPythiaHydjet]; iJetPt <= lastStudiedJetPtBinEEC[kPythiaHydjet]; iJetPt++){
          ...
          if(drawReflectedConeToHydjetRatio){
            hReflectedConeToHydjetRatio[iParticleDensity][iCentrality][iJetPt][iTrackPt] = (TH1D*) hParticleDensity[iParticleDensity][iCentrality][iJetPt][iTrackPt][EECHistograms::kReflectedCone][EECHistograms::knSubeventTypes]->Clone(Form("reflectedConeToHydjetRatio%d%d%d%d", iParticleDensity, iCentrality, iJetPt, iTrackPt));
